...
app.get("/secrets", async (req, res) => {
  if (req.isAuthenticated()) {
    try {
      ...
      const userSecret = result.rows[0]?.secret || "No secret";
...
app.post("/submit", async (req, res) => {
  ...
  try {
    const secret = req.body.secret; // Отримуємо секрет із форми
    await db.query("UPDATE users SET secret = $1 WHERE id = $2", [secret, req.user.id]);

    res.redirect("/secrets"); // Перенаправляємо на сторінку з секретами
  } catch (err) {
    console.error("Err", err);
    res.status(500).send("Status err");
  }
